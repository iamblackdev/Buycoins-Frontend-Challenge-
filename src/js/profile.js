  ...
  fetchData = () => {
    ...
    fetch("https://api.github.com/graphql", {
      ...
      .then(fetchedData => {
        if (
          !fetchedData.errors &&
          fetchedData.data.repositoryOwner &&
          Object.entries(fetchedData.data.repositoryOwner).length !== 0
        ) {
          ...
          names.forEach(
            names => (names.innerText = fetchedData.data.repositoryOwner.name)
          );

          // OUTPUTING USERNAME TO THE DOM
          bio.forEach(
            bio => (bio.innerText = fetchedData.data.repositoryOwner.bio)
          );
          ...
          counters.forEach(
            counter =>
              (counter.innerText =
                fetchedData.data.repositoryOwner.repositories.totalCount)
          );
          ...
          fetchedData.data.repositoryOwner.repositories.edges.forEach(
            repository =>
...
let stickyElement = () => {
  ...
  let stickyHeaderMobile = document
    .querySelector(".main-header.mobile")
    .getBoundingClientRect();

  // GET THE USER DETAILS ABOVE THE  MAIN HEADER MOBILE
  let userDetails = document
